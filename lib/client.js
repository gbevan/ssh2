...
Client.prototype.connect = function(cfg) {
  ...
  // option to call hostVerifier callback with an object
  // {key, keyBase64, hash, randomart}
  // instead of just a string containing the hash
  // defaults to old behaviour
  cfg.hostVerifierKeyObj = cfg.hostVerifierKeyObj ? true : false;
  ...
  if (typeof cfg.hostVerifier === 'function') {
    ...
    stream.once('fingerprint', function(key, verify) {
      ...
      var ret;
      if (cfg.hostVerifierKeyObj) {
        // provide hash as base64 to match openssh
        hasher = crypto.createHash(cfg.hostHash);
        hasher.update(key);
        var hashBase64 = hasher.digest('base64');

        // provide key in base64 format too
        var keyBase64 = Buffer.from(key, 'binary').toString('base64');

        // generate randomart
//        var buf = Buffer.from(key, 'binary');
        var buf = Array.prototype.slice.call(key, 0);
